...
class AdhanProvider with ChangeNotifier {
  ...
  AdhanProvider(
      this.adhanDependencyProvider, this.locationInfo,)
  ...
  Adhan createAdhan(
    ...
    return Adhan(
      type: type,
      title:tr(getAdhanName(type, isJummah: startTime.isJummahToday)),
      // title: appLocalization.getAdhanName(type, isJummah: startTime.isJummahToday),
      startTime: startTime,
      endTime: endTime,
      ...
      localCode: appContext?.locale.languageCode??'ar',
  ...
  List<Adhan> getAdhanData(DateTime date) {
    ...
    // print('getAdhanData ${list.length}');
    return list;
  }
}
