                                    ...
                                    child: Container(
                                      ...
                                      child: c.state.isDownloading.value
                                          ? GetX<AudioCtrl>(builder: (c) {
                                              final data = c.state.tmpDownloadedAyahsCount;
                                              log('$data => REBUILDING  ${audioCtrl.state.tmpDownloadedAyahsCount}');
                                              return PackageSliderWidget.downloading(
                                                  currentPosition: data,
                                                  filesCount: audioCtrl.currentAyahFileName.length,
                                                  activeTrackColor: style!.seekBarActiveTrackColor ?? Theme.of(context).primaryColor,
                                                  inactiveTrackColor: style!.seekBarInactiveTrackColor ?? Theme.of(context).hintColor,
                                                  thumbColor: style!.seekBarThumbColor ?? Theme.of(context).primaryColor,
                                                  horizontalPadding: style!.seekBarHorizontalPadding ?? 0);
                                            })
                                              ...
                                              builder: (context, snapshot) {
                                                if (snapshot.hasData) {
                                                  ...
                                                  return PackageSliderWidget.player(
                                                    horizontalPadding: style!.seekBarHorizontalPadding ?? 0.0,
                                                    duration: positionData?.duration ?? Duration.zero,
                                                    position: positionData?.position ?? Duration.zero,
                                                    activeTrackColor: style!.seekBarActiveTrackColor ?? Theme.of(context).primaryColor,
                                                    inactiveTrackColor: style!.seekBarInactiveTrackColor ?? Theme.of(context).highlightColor,
                                                    thumbColor: style!.seekBarThumbColor ?? Theme.of(context).primaryColor,
