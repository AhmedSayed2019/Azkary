        ...
        children: <Widget>[
          Padding(
            ...
            child: SliderTheme(
              data: SliderTheme.of(context).copyWith(
                activeTrackColor: widget.activeTrackColor ?? Theme.of(context).primaryColor,
                inactiveTrackColor: widget.inactiveTrackColor ?? Theme.of(context).highlightColor,
                thumbColor: widget.thumbColor ?? Theme.of(context).primaryColor,
                activeTickMarkColor: Theme.of(context).primaryColor,
                disabledSecondaryActiveTrackColor:Theme.of(context).primaryColor ,
                valueIndicatorColor: Theme.of(context).primaryColor,
                valueIndicatorStrokeColor: Theme.of(context).primaryColor,
              ...
              child: Slider(
                min: 0.0,
                max: widget.duration?.inMilliseconds.toDouble() ?? widget.filesCount!,
                value: _sliderValue,
          ...
          Transform.translate(
            ...
            child: widget.filesCount != null
              ...
              decoration:  BoxDecoration(color: Theme.of(context).primaryColor, borderRadius: const BorderRadius.all(Radius.circular(4),),
                    ),
                    child: Text(
                      'downloading'.tr,
                ...
                : Row(
                    ...
                    children: [
                      Container(
                        ...
                        child: Text(
                          (RegExp(r'((^0*[1-9]\d*:)?\d{2}:\d{2})\.\d+$').firstMatch("$remaining")?.group(1) ?? '$remaining').convertArabicToEnglishNumbers,
                          style: QuranLibrary().naskhStyle.copyWith(color: widget.textColor ?? Colors.white, fontSize: 16,),
                        ),
                      ),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8.0, vertical: 2.0),
                        ...
                        child: Text((RegExp(r'((^0*[1-9]\d*:)?\d{2}:\d{2})\.\d+$').firstMatch("$total")?.group(1) ?? '$total').convertArabicToEnglishNumbers,
                          style: QuranLibrary().naskhStyle.copyWith(color: widget.textColor ?? Colors.white, fontSize: 16,),
                        ),
                      ),
                    ],
