...
extension DoubleHelper<T extends num> on double {
  ...
  String getHeadingName(double heading) {
    return (heading >= 0 && heading < 45)
        ? tr("compass_heading_north")
        : (heading >= 45 && heading < 90)
        ?  tr("compass_heading_north_east")
        : (heading >= 90 && heading < 135)
        ?  tr("compass_heading_east")
        : (heading >= 135 && heading < 180)
        ?  tr("compass_heading_south_east")
        : (heading >= 180 && heading < 225)
        ?  tr("compass_heading_south")
        : (heading >= 225 && heading < 270)
        ?  tr("compass_heading_south_west")
        : (heading >= 270 && heading < 315)
        ?  tr("compass_heading_west")
        : (heading >= 315 && heading < 360)
        ?  tr("compass_heading_north_west")
        :  tr("compass_heading_north");
  }
...
extension DateHelper on DateTime {
  ...
  String getHizriDateForLocale( {String separator = '-'}){

    final months = [
      tr("arabic_month_muharram"),
      tr("arabic_month_safar"),
      tr("arabic_month_rabi_al_awwal"),
      tr("arabic_month_rabi_al_thani"),
      tr("arabic_month_jumada_al_awwal"),
      tr("arabic_month_jumada_al_thani"),
      tr("arabic_month_rajab"),
      tr("arabic_month_shaban"),
      tr("arabic_month_ramadan"),
      tr("arabic_month_shawwal"),
      tr("arabic_month_dhu_al_qadha"),
      tr("arabic_month_dhu_al_hijjah")];

    final hDate = HijriCalendar.fromDate(this);
    return '${hDate.hDay.localizeTo()}$separator${months[hDate.hMonth-1]}$separator${hDate.hYear.localizeTo()}';
  }

  String localizeTimeTo (){
    int hr = hour;
    bool isPM = false;
    if(hr > 12){
      hr -= 12;
    ...
    return '${digitHr == 1 ? '0' : ''}$hr:${digitMin == 1 ? '0' : ''}$minute ${isPM ? tr('pm') : tr('am')}'/*.localizeDigitsOnly(locale)*/;
...
extension IntHelper on int{
  ...
  String getDigitFor(int i){
    ...
    return [
      tr("zero"),  tr("one"), tr("two"),  tr("three"),  tr("four"),  tr("five"),  tr("six"),  tr("seven"),  tr("eight"),  tr("nine")
    ][i];
  }
  ...
  String localizeTo(){
    ...
    for(int i = 0; i < strNum.length; i++){
      ...
      final localizedDigit = getDigitFor(digit.toInt());
      localized += localizedDigit;
    }

    if(tr('numeric_direction') == 'rlt'){
      localized = localized.split('').reversed.join();
    }
