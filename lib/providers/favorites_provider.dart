...
class FavoritesProvider with ChangeNotifier {
  ...
  int newId(int tableIndex) {
    return (_favorites[_tables[tableIndex]]??[]).isEmpty ? 0 : (_favorites[_tables[tableIndex]]??[])[length(tableIndex) - 1].id + 1;
  }
  ...
  Future<bool> addFavorite(int tableIndex, int itemId) async {
    try {
      await databaseHelper.insert(_tables[tableIndex], FavoriteModel(id: newId(tableIndex), itemId: itemId).toMap());
      _favorites[_tables[tableIndex]]!.add(FavoriteModel(id: newId(tableIndex),itemId:  itemId));
      print('addFavorite');
      notifyListeners();
      return true;
    } catch (e) {
