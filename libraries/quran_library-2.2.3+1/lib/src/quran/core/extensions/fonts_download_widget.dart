...
extension FontsDownloadWidgetExtension on QuranCtrl {
  ...
  Widget fontsDownloadWidget(BuildContext context,
    ...
    // Theming fallbacks
    final Color accent = downloadFontsDialogStyle?.linearProgressColor ?? Theme.of(context).primaryColor;
    final Color background = downloadFontsDialogStyle?.linearProgressColor ?? AppColors.getBackgroundColor(isDark);
    final Color textColor = downloadFontsDialogStyle?.titleColor ?? AppColors.getTextColor(isDark);
    final Color notesColor = downloadFontsDialogStyle?.notesColor ?? AppColors.getTextColor(isDark);
    final Color dividerColor = downloadFontsDialogStyle?.dividerColor ?? accent;
    final Color outlineColor = (downloadFontsDialogStyle?.downloadButtonBackgroundColor != null)
        ? downloadFontsDialogStyle!.downloadButtonBackgroundColor!.withValues(alpha: .2)
        : isDark ? accent.withValues(alpha: .35) : accent.withValues(alpha: .2);
                ...
                if (preparing || downloading)
                  ...
                  IconButton(
                    ...
                    onPressed: () async {
                      ...
                      } else if (!ctrl.state.isDownloadingFonts.value && !ctrl.state.isPreparingDownload.value) {
