...
public class FlutterOverlayWindowPlugin implements
    ...
    /**
      * Azkary patch: cache the overlay engine as soon as the plugin attaches to
      * ANY engine — including the headless one android_alarm_manager_plus spins
      * up while the app is closed. Upstream only did this in
      * onAttachedToActivity, so with no Activity the cache stayed empty and
      * OverlayService's field initialisers NPE'd on getDartExecutor().
      */
    private static boolean creatingOverlayEngine = false;

    private void ensureOverlayEngine() {
        if (context == null) return;
        if (FlutterEngineCache.getInstance().get(OverlayConstants.CACHED_TAG) != null) return;
        // Re-entrancy guard: createAndRunEngine registers plugins on the new
        // engine, which calls onAttachedToEngine again while the cache is still
        // empty. Without this the engines multiply without bound.
        if (creatingOverlayEngine) return;
        creatingOverlayEngine = true;
        try {
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        ...
        if (call.method.equals("checkPermission")) {
            ...
            // Azkary patch: build the overlay engine here rather than only in
            // onAttachedToActivity, so a headless isolate (alarm with the app
            // closed) can show the overlay too.
            ensureOverlayEngine();
    ...
    public void onMessage(@Nullable Object message, @NonNull BasicMessageChannel.Reply reply) {
        // Azkary patch: guard the cache lookup — null when no engine exists yet.
