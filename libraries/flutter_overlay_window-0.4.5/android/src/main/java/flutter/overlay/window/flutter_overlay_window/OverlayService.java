...
public class OverlayService extends Service implements View.OnTouchListener {
    ...
    // Azkary patch: lazy, null-safe. These were field initialisers that
    // dereferenced the engine cache at construction time; with the app closed
    // the cache is empty and the service crashed the whole process with an NPE.
    private MethodChannel flutterChannel;
    private BasicMessageChannel<Object> overlayMessageChannel;

    private FlutterEngine cachedEngine() {
        return FlutterEngineCache.getInstance().get(OverlayConstants.CACHED_TAG);
    }

    private MethodChannel flutterChannel() {
        FlutterEngine engine = cachedEngine();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        FlutterEngine engine = cachedEngine();
        if (engine == null) {
            // Azkary patch: no cached engine means the overlay cannot be shown.
            // Bail out instead of NPE-ing and taking the whole process down.
            Log.e("OverlayService", "No cached overlay engine; aborting start");
            isRunning = false;
            stopSelf();
